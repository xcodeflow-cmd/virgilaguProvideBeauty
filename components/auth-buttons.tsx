    ...
    return (
      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
        <Button asChild variant="ghost" className="inline-flex rounded-full border border-white/10 bg-white/[0.03] px-4 sm:px-5">
          <Link href="/auth/signin" onClick={onNavigate}>Autentificare</Link>
        </Button>
        <Button
          asChild
        ...
        >
          <Link href="/live" onClick={onNavigate}>Vezi LIVE</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
      {session.user.role === "ADMIN" ? (
        <Button asChild variant="ghost" className="inline-flex rounded-full border border-white/10 bg-white/[0.03] px-4 sm:px-5">
          <Link href="/admin" onClick={onNavigate}>Admin</Link>
        </Button>
      ) : null}
      <Button asChild variant="secondary" className="inline-flex border-[#d6b98c]/[0.24] bg-[#d6b98c]/[0.08] px-4 text-[#f4e5c8] sm:px-5">
        <Link href="/dashboard" onClick={onNavigate}>Contul meu</Link>
      </Button>
      <SignOutButton onComplete={onNavigate} />
    </div>
  );
}
