    ...
    <article className="glass-panel overflow-hidden rounded-[1.75rem] border border-white/10">
      <div className="relative aspect-[16/10]">
        ...
        <div className="absolute left-4 top-4 inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/45 px-3 py-1 text-xs uppercase tracking-[0.3em] text-white">
          {item.isLive ? <Radio className="h-3.5 w-3.5 text-red-500" /> : <Video className="h-3.5 w-3.5 text-gold-light" />}
          {item.isLive ? "Live now" : item.visibility === "SUBSCRIBERS" ? "Members" : "Single access"}
        </div>
      </div>
      <div className="space-y-5 p-6">
        <div>
          <p className="text-xs uppercase tracking-[0.35em] text-gold-light/80">{formatDate(item.scheduledFor)}</p>
